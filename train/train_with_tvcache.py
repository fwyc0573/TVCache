...
import json
from tvc_agent_loop import VideoAgentLoop, VideoSandboxEnv
...
class Config:
    ...
    log_path: str = "./tests/rebuttal_cache"
...
async def _run_training(
    config: Config,
    cache_lifecycle: TVCacheRunLifecycle,
):
    rollout_log_dir = os.path.join(config.log_path, "rollouts")
    run_id = f"tvcache_{time.time_ns()}"
    ...
    while batch_idx < n_train_batches:
        ...
        for d_idx, data in enumerate(batch_rows):
            ...
            for i in range(config.group_size):
                ...
                if d_idx == 0 and i == 0:
                    logger.debug(f'STORED NEXT BATCH: {next_batch_task_ids} for {d_idx} and {i}')
                    arg_data["next_batch"] = next_batch_task_ids
                    arg_data["rollout_count"] = config.group_size
                
                task_id = get_task_id(arg_data['question'])
                cache_lifecycle.register_task(task_id)
                agent_loop = VideoAgentLoop(
                    training_data_point=arg_data,
                    sampling_client=sampling_client,
                    num_turns=config.num_turns,
                    renderer=renderer,
                    sandbox_base_url=config.sandbox_base_url,
                    tvcache_base_url=config.tvcache_base_url,
...
async def main(config: Config):
    async with TVCacheRunLifecycle(
        environment_class=VideoSandboxEnv,
        tvcache_base_url=config.tvcache_base_url,
        env_kwargs={"sandbox_base_url": config.sandbox_base_url},
    ) as cache_lifecycle:
        await _run_training(config, cache_lifecycle)
