...
Future execution must choose an authorized API endpoint, select and record 8–12 W1 tasks and image digests, define two concrete W2-T1 instances, and decide where raw pilot artifacts are stored. P00 must also decide whether to refresh the CPU baseline or cite the prior reviewed baseline. These are execution-time decisions and are intentionally not fabricated here.
