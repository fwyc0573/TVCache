...
class DataBase:
    def __init__(self, video_path, base_dir='preprocess', use_reid=True, model_dir=None):
        ...
        # Use sandbox-specific database path to avoid conflicts
        self.db_path = os.path.join(self.video_dir, 'database.db')
        if os.path.exists(self.db_path):
            # os.remove(self.db_path)
            print(f'Skipping database creation in {self.video_dir}')
            return
    ...
    def query_database(self, program):
        ...
        try:
            ...
            # CRITICAL: Always close cursor and connection to prevent resource leak
            cursor.close()
            connection.close()
